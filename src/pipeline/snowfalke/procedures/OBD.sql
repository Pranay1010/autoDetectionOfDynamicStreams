CREATE OR REPLACE PROCEDURE MY_DB.TEST_SCHEMA.LOAD_OBD_PROCEDURE()
RETURNS STRING
LANGUAGE PYTHON
RUNTIME_VERSION = '3.8'
...
def process_record(transposed_df, record_content):
    ...
    for i in range(transposed_df.shape[1]):
        ...
        if pd.notnull(transposed_df.iloc[0, i]): # Check if SPN is not null
            ...
            # Handle LAMPS column
            lamps_entries = transposed_df.iloc[4, i]
...
def update_table_schema(session, new_columns):
    existing_columns_query = '''
        ...
        WHERE TABLE_NAME = 'OBD_DETAIL_TABLE'
...
def main(session: snowpark.Session):
    ...
    try:
        # Query to fetch data
        DR =  '''SELECT * FROM "MY_DB"."TEST_SCHEMA"."OBD_TABLE"
            WHERE RECORD_METADATA:"SnowflakeConnectorPushTime"::string::timestamp_ntz < DATEADD(DAY, -1, CURRENT_DATE())
            AND RECORD_METADATA:"SnowflakeConnectorPushTime" IS NOT NULL
            AND REPLACE(RECORD_CONTENT:"eventId", '"','')::string NOT IN (
                SELECT DISTINCT EVENT_ID FROM "MY_DB"."TEST_SCHEMA"."OBD_DETAIL_TABLE"
            )
            LIMIT 2000
        '''
        ...
        for record in dataframe.to_local_iterator():
            ...
            if all_consolidated_rows:
                ...
                new_snowpark_df.write.save_as_table("MY_DB.TEST_SCHEMA.OBD_DETAIL_TABLE", mode="append")
...
call MY_DB.TEST_SCHEMA.LOAD_OBD_DETAIL()